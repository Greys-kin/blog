...
import { useRegisterANewUserMutation } from "../../api/blog-api";
...
const SignUp = () => {
  ...
  const [registerUser, { isError, error, isSuccess }] =
  ...
  const errorUsernameMessage =
    isFetchBaseQueryError(error) && error.data.errors?.username
      ? `Username ${error.data.errors.username}`
      : "";

  const errorEmailMessage =
    isFetchBaseQueryError(error) && error.data.errors?.email
  ...
  return (
    ...
    >
      ...
      <button
        type="submit"
        className="btn btn-primary d-grid gap-2 col-12 mx-auto"
      >
        Create
      </button>
      {serverError(
      ...
      <div
        style={{ fontSize: 12, textAlign: "center", color: "#8C8C8C" }}
        className="mt-3"
      >
